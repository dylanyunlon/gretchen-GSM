...
void Graph::loadGraphFromFile(const std::string& file_path, std::unordered_map<VertexID, VertexID>& part2data) {
    std::ifstream infile(file_path);
    if (!infile.is_open()) {
        std::cout << "Can not open the graph file " << file_path << " ." << std::endl;
        exit(-1);
    }
    std::unordered_map<VertexID, VertexID> data2part;
    char type;
    infile >> type >> vertices_count_ >> edges_count_;
    offsets_ = new ui[vertices_count_ +  1];
    offsets_[0] = 0;
    ...
    while (infile >> type) {
        if (type == 'v') { // Read vertex.
            ...
            ui degree;
            infile >> id >> label >> degree;
            data2part[id] = node_cnt;
            part2data[node_cnt] = id;
            id = node_cnt;
            node_cnt += 1;
            labels_[id] = label;
            offsets_[id + 1] = offsets_[id] + degree;
